...
class AbstractRobot(object):
    ...
    def _fill_states(self, now, rr_robot_state, rr_advanced_robot_state, rr_state_sensor_data):
        ...
        ### ?. operator
        sensor_data_header = SensorDataUtil.FillSensorDataHeader(RRN,_robot_info?.device_info, _state_seqno)
        
        sensor_data = new RobotStateSensorData()
    ...
    def _verify_communication(self,now):
        ...
        if ((now - self._last_joint_state > self._communication_timeout)
            ...
            self._controller_state = RobotControllerState.undefined
            ### double[0]
            self._joint_position = new double[0]
            self._joint_velocity = new double[0]
            self._joint_effort = new double[0]
    ...
    def _fill_robot_command(self, now, joint_pos_cmd, joint_vel_cmd):
        ...
        if self._command_mode==RobotCommandMode.jog:
            ...
            ts=TimeSpec()
            ep = default
            if (not(rrposition_command?.TryGetInValue(pos_cmd, ts, ep) ?? False)):
            ...
            ep = default
            ###??
            if (not(rrvelocity_command?.TryGetInValue(vel_cmd, ts, ep) ?? False))
            ...
            if (vel_cmd
                ...
                or (np.abs(vel_cmd.state_seqno - _state_seqno) > 50)
                or (vel_cmd.command.Length != _joint_count)
                or (vel_cmd.units.Length != 0 and vel_cmd.units.Length != _joint_count))
    ...
    def RRServiceObjectInit(self, context, service_path):
        ...
        self._broadcast_downsampler = new BroadcastDownsampler(context, 0)
